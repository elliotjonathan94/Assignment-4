...
mysql -u"$MYSQLUSER" -p"$MYTABLE" -e"CREATE TABLE $MYTABLE Hometown VARCHAR(255), Genre VARCHAR(255), Weather VARCHAR(255), Animals VARCHAR(255), Flower VARCHAR(255), ALTER TABLE $MYTABLE ADD PRIMARY KEY (Hometown)" $MYDATABASE
fi
# Write data from tmp.csv to database table
echo "Writing information from $MYTABLE to $MYDATABASE"
